...
function register($conn){
	...
	if(isset($_REQUEST['submit'])){
	  ...
	  mysqli_query($conn,"INSERT INTO user (name, mobile, email, password) VALUES ('$name','$mobile','$email','$password')");
...
function forgetPassword($conn){
 ...
 $sql = "UPDATE adminlogin SET password='$mypassword', username='$username' WHERE mobile='$mobile_no'";
	    mysqli_query($conn, $sql);
...
function unblockuser($conn){
	...
	if($_POST){
		...
		$id = $_POST['no'];
		$sql = "UPDATE register SET block = '1' WHERE no = '$id'";
		mysqli_query($conn, $sql);
